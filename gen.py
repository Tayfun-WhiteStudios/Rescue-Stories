...
(function(){
  ...
  // Smooth ease-out-quart — elegant deceleration, no bounce
  function easeOutQuart(x){return 1-Math.pow(1-x,4);}
  // Bone grid — larger treats, natural rotation variance, organic scatter
  var L=10,er=5.8,ov=3.6,sw=3.8; // bone geometry constants
  var vSp=17,hSp=36,allK=[];
  for(var row=0;row<12;row++){var baseY=CY+R-8-row*vSp;var xOff=(row%2)*hSp*0.5;for(var col=-3;col<=3;col++){var kx=CX+col*hSp+xOff+(Math.random()-0.5)*5;var ky=baseY+(Math.random()-0.5)*4;var dist=Math.sqrt((kx-CX)*(kx-CX)+(ky-CY)*(ky-CY));if(dist+16<R-4){allK.push({fx:kx,fy:ky,rot:(Math.random()-0.5)*1.1,sh:Math.random()});}}}
  allK.sort(function(a,b){return b.fy-a.fy;});
  var active=[];
  function spawnK(tmpl){active.push({x:tmpl.fx,fy:tmpl.fy,cy:tmpl.fy-120,rot:tmpl.rot,sh:tmpl.sh,p:0});}
  function drawBone(k){
    ctx.save();ctx.translate(k.x,k.cy);ctx.rotate(k.rot);
    var s=k.sh;
    // Warm caramel — diagonal linear gradient, upper-left bright → lower-right dark
    var g=ctx.createLinearGradient(-L-er,-ov-er,L+er,ov+er);
    g.addColorStop(0,'rgb('+(232+Math.round(s*8))+','+(148+Math.round(s*14))+','+(42+Math.round(s*10))+')');
    g.addColorStop(0.44,'rgb('+(188+Math.round(s*10))+','+(100+Math.round(s*14))+','+(22+Math.round(s*8))+')');
    g.addColorStop(1,'rgb('+(108+Math.round(s*12))+','+(55+Math.round(s*10))+','+(10+Math.round(s*6))+')');
    // Soft ambient shadow (painted ellipse — no canvas shadow blur needed)
    ctx.fillStyle='rgba(0,0,0,0.36)';
    ctx.beginPath();ctx.ellipse(0.5,2.8,L+er*0.8,ov+er*0.55,0,0,Math.PI*2);ctx.fill();
    // Bone body
    ctx.fillStyle=g;
    ctx.beginPath();ctx.rect(-L,-sw,L*2,sw*2);ctx.fill();
    ctx.beginPath();ctx.arc(-L,-ov,er,0,Math.PI*2);ctx.fill();
    ctx.beginPath();ctx.arc(-L, ov,er,0,Math.PI*2);ctx.fill();
    ctx.beginPath();ctx.arc( L,-ov,er,0,Math.PI*2);ctx.fill();
    ctx.beginPath();ctx.arc( L, ov,er,0,Math.PI*2);ctx.fill();
    // Broad soft specular on each knob top — the material catch-light
    ctx.fillStyle='rgba(255,245,215,0.45)';
    ctx.beginPath();ctx.ellipse(-L-er*0.14,-ov-er*0.18,er*0.46,er*0.28,-0.28,0,Math.PI*2);ctx.fill();
    ctx.beginPath();ctx.ellipse( L-er*0.14,-ov-er*0.18,er*0.46,er*0.28,-0.28,0,Math.PI*2);ctx.fill();
    // Crisp micro-specular dot — makes it look lacquered, ceramic
    ctx.fillStyle='rgba(255,255,248,0.84)';
    ctx.beginPath();ctx.arc(-L-er*0.26,-ov-er*0.30,er*0.12,0,Math.PI*2);ctx.fill();
    ctx.beginPath();ctx.arc( L-er*0.26,-ov-er*0.30,er*0.12,0,Math.PI*2);ctx.fill();
    ctx.restore();
  }
  function frame(){
    ctx.clearRect(0,0,W,H);
    fill+=(TARGET-fill)*0.010;
    // Reveal bones whose resting position is below the fill line — same geometry as water
    var fillLine=CY+R*(1-fill*2);
    var show=0;for(var i=0;i<allK.length;i++){if(allK[i].fy>=fillLine)show++;else break;}
    ...
    ctx.fillStyle=bg;ctx.fillRect(0,0,W,H);
    // Animate bones — smooth ease-out fall, slower & more deliberate
    active.forEach(function(k){k.p=Math.min(1,k.p+0.026);k.cy=k.fy-120*(1-easeOutQuart(k.p));drawBone(k);});
    // Warm pile-surface glow — the signature Apple touch: one slowly breathing light
    if(active.length>4){
      var pt=Math.min.apply(null,active.slice(0,active.length).map(function(k){return k.cy-(ov+er);}));
      var glowX=CX+Math.sin(t*0.11)*R*0.3;
      var pg=ctx.createRadialGradient(glowX,pt+4,0,glowX,pt+4,R*0.62);
      pg.addColorStop(0,'rgba(210,120,22,0.12)');pg.addColorStop(0.5,'rgba(200,105,18,0.05)');pg.addColorStop(1,'rgba(180,80,10,0)');
      ctx.fillStyle=pg;ctx.fillRect(CX-R,pt-8,R*2,38);
    }
    // Glass wall reflection
    var gl=ctx.createRadialGradient(CX-R+8,CY-12,0,CX-R+6,CY,R*0.38);gl.addColorStop(0,'rgba(255,255,255,0.07)');gl.addColorStop(1,'rgba(255,255,255,0)');
    ctx.fillStyle=gl;ctx.fillRect(0,0,W,H);
    ctx.restore();
    // Ring — identical to water version
    ctx.beginPath();ctx.arc(CX,CY,R,0,Math.PI*2);ctx.strokeStyle='rgba(255,255,255,0.07)';ctx.lineWidth=6;ctx.stroke();
    ctx.beginPath();ctx.arc(CX,CY,R-3,0,Math.PI*2);ctx.strokeStyle='rgba(255,255,255,0.06)';ctx.lineWidth=1;ctx.stroke();
    ctx.beginPath();ctx.arc(CX,CY,R+3,0,Math.PI*2);ctx.strokeStyle='rgba(255,255,255,0.03)';ctx.lineWidth=1;ctx.stroke();
    ctx.save();ctx.translate(CX,CY);ctx.rotate(-Math.PI/2);
